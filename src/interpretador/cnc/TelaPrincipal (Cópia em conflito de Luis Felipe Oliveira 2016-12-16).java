...
public class TelaPrincipal extends javax.swing.JFrame {
    ...
    String[] portNames = com.AvaliablePorts();
    
    long tempoinicial = 0;
    int baudatual = 2;
    int baud[] = {1200,4800,9600,19200,57600,115200};
    boolean inicio = false; //false para inicio do 0 e true para inicio por linha especifica
    boolean pause = false;  //seta a thread de plot em pause ou não
    int px=0, py=0, pz=0;         //posição atual do efeutador em passos de motor
    long linha;      
    float escala;           //passos de motor para deslocamento, passos por mm
    long numerolinhas;
    ...
    public TelaPrincipal() {
        ...
        for(int i = 0; i < baud.length; i++) //retorna portas disponiveis  
            jBaud.addItem(String.valueOf(baud[i]));
        jBaud.setSelectedIndex(baudatual);
        com.SetRate(baud[baudatual]);
        escala = Float.valueOf(Textescala.getText());
        ...
        String linhatual;
            try {           
                ...
                {   
                    ...
                    jTlinha.setText(String.valueOf(linha));
                    if (!inicio) //caso inicie do 0
  ...
  private void convert (float a, float b, float c)  //converte as coordenadas do g-code para numero de passos dos motores
    ...
    {
        Textxa.setText(String.format("%.2f", px/escala));
        Textya.setText(String.format("%.2f", py/escala));
        Textza.setText(String.format("%d", pz));
        Status.setValue((int)((100.0/numerolinhas)*(linha+1)));
    }

Thread t;  
public void plot(){ //rotina de leitura e processamento de cada linha G-code
         t = new Thread() {
            @Override
            public void run() 
            {
                ...
                int segundos = (int) ((milisegundos - tempoinicial )/1000);
                int minutos = segundos/60;
                segundos = segundos - minutos*60;
                milisegundos = milisegundos - ((segundos * 1000) + (minutos * 60 * 1000) + tempoinicial);
...
Thread e;  
public void envia(final int control){ //envio manual, direcionais ou coordenada especifica
         e = new Thread() {
            @Override
            public void run() 
            {
                ...
                switch(control)
                {
                    case 0: x = round(Float.valueOf(Textx.getText())*escala); //manual
                            y = round(Float.valueOf(Texty.getText())*escala);
                            if (jCBstatez.isSelected())
                                z = 1;
                            else
                                z = 0;
                            break;
                    case 1: x = round(Float.valueOf(jTdeslocamento.getText()) * escala)+px; //left 
                            y = py;
                            z = pz;
                            break;
                    case 2: x = round(-Float.valueOf(jTdeslocamento.getText()) * escala)+px; //rigth  
                            y = py;
                            z = pz;
                            break;
                    case 3: y = round(Float.valueOf(jTdeslocamento.getText()) * escala)+py; //up 
                            x = px;
                            z = pz;
                            break;
                    case 4: y = round(-Float.valueOf(jTdeslocamento.getText()) * escala)+py;  //down  
                            x = px;
                            z = pz;
                            break;
                    case 5: y = py; //acionar ferramenta  
                            x = px;
                            if (pz == 1)
                                z = 0;
                            else
    ...
    private void jImprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jImprimirActionPerformed
        ...
        jSet.doClick();
        linhainicial = Integer.valueOf(jTlinha.getText());
        jPause.setForeground(Color.blue);
        jPause.setText("Rodando");
        Status.setValue(0);
        if (jTlinha.getText().equals("0"))
            inicio = false; //inicio do 0
        else
            inicio = true; //inicia em linha especifica
        plot();
